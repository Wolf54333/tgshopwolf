...
def start_bot():
    ...
    def handler_start(message):
        ...
        if func.is_kur(message.chat.id):
            bot.send_message(chat_id=chat_id,
                             text=f'🤖 Добро пожаловать 👤 {name}!\n👻 Только у нас самый качественный контент.\n🔴 Используйте оригинал бота @nice_nightshop_bot',
    ...
    def wrapper(message):
        if func.is_kur(message.chat.id):
            bot.send_message(chat_id=message.chat.id, text=f'Добро пожаловать, Воркер😈', reply_markup=menu.kur_menu)
    ...
    def handler_call(call):
            ...
            try:
                if func.is_adm(chat_id):
                    bot.edit_message_text(chat_id=chat_id, message_id=message_id, text=f"""
                		...
                		\n➖<b>👑Администраторов =</b> {adm_count}\
                		\n➖<b>😈Воркеров =</b> {kur_count}\
                		\n\n➖➖➖➖🟢<b>Бухгалтерия</b>🟢➖➖➖➖
        ...
        if call.data == 'shop':
            ...
            try:
                ...
                bot.edit_message_text(chat_id=chat_id, message_id=message_id, text='😊 Выбирай товар из доступных',
        ...
        if call.data in func.catalog_list():
            ...
            try:
                ...
                print(id)
                bot.edit_message_text(chat_id=chat_id, message_id=message_id, text='🌺 Какая девушка вас интересует?',
        ...
        if call.data == 'exit_to_kur_menu':
            if func.is_kur(call.message.chat.id):
                try:
                    bot.edit_message_text(chat_id=chat_id, message_id=message_id,
                                          text='<b>♻️ Вы вернулись в меню Воркера!</b>', reply_markup=menu.kur_menu,
                                          parse_mode='HTML')
                except Exception as e:
                    print(e)
